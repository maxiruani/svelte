...
| `css` | `"internal"`  | If `"internal"` (formerly `true`, also accepted but deprecated), styles will be included in the JavaScript class and injected at runtime for the components actually rendered. If `"external"` (formerly `false`, also accepted but deprecated), the CSS will be returned in the `css` field of the compilation result. If `"none"`, the CSS will be completely ignored and not processed, useful when generating hydratable DOM code. Most Svelte bundler plugins will set this to `"external"` and use the CSS that is statically generated for better performance, as it will result in smaller JavaScript bundles and the output can be served as cacheable `.css` files.
